...
RUN cp /usr/lib64/libcob.so.4 \
       ...
       /cobol/lib
RUN cd /cobol && \
      COB_LDFLAGS="-Wl,--rpath=./lib -Wl,--dynamic-linker=./lib/ld-linux-x86-64.so.2" cobc -x pastime.cob
RUN cd /cobol && zip -r /lambda-cobol.zip ./bootstrap ./pastime ./lib
